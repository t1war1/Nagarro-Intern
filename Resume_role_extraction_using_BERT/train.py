...
flags.DEFINE_bool("use_tpu", config.use_tpu, "Whether to use TPU or GPU/CPU.")
...
def train(train_file, eval_file):
    ...
    train_examples = processor.get_train_examples(FLAGS.data_dir, train_file)
    num_train_steps = int(
    ...
    pc.file_based_convert_examples_to_features(
        train_examples, label_list, FLAGS.max_seq_length, tokenizer, train_file)
    tf.logging.info("***** Running training *****")
    tf.logging.info("  Num examples = %d", len(train_examples))
    tf.logging.info("  Batch size = %d", FLAGS.train_batch_size)
    tf.logging.info("  Num steps = %d", num_train_steps)
    train_input_fn = pc.file_based_input_fn_builder(
    ...
    if FLAGS.do_eval:
        eval_examples = processor.get_dev_examples(FLAGS.data_dir, eval_file)
        num_actual_eval_examples = len(eval_examples)
        if FLAGS.use_tpu:
            while len(eval_examples) % FLAGS.eval_batch_size != 0:
                eval_examples.append(pc.PaddingInputExample())
        eval_file = os.path.join(FLAGS.output_dir, "eval.tf_record")
        pc.file_based_convert_examples_to_features(eval_examples, label_list, FLAGS.max_seq_length, tokenizer, eval_file)
        ...
        eval_input_fn = pc.file_based_input_fn_builder(input_file=eval_file,seq_length=FLAGS.max_seq_length,is_training=False,drop_remainder=eval_drop_remainder)
